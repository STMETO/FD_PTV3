...
def visualize_flexible_pointclouds(coords, window_configs, scene_id, args):
    try:
        ...
        if args.look_down and initial_azimuth is None:
            # 俯视模式：设置俯视角度 (elevation=0 即从正上方向下看)
            initial_azimuth = 0.0
...
def process_and_visualize_flexible(coords_path, args, scene_id,
    ...
    try:
        ...
        # --- LookDown 俯视模式：确定天花板掩码 ---
        ceiling_mask = None
        gt_labels_loaded = None  # 提前加载语义标签，供后续复用
        window_types_list = [w.strip().lower() for w in args.windows.split(',')] if args.windows else []
        if args.look_down or any(w in window_types_list for w in ['gt', 'pred1', 'pred2']):
            if gt_path and os.path.exists(gt_path):
                gt_labels_loaded = np.load(gt_path)
                if gt_labels_loaded.ndim > 1:
                    gt_labels_loaded = gt_labels_loaded.flatten()

        if args.look_down:
            if gt_labels_loaded is not None:
                # 基于语义标签过滤天花板（标签0 = 天花板）
                ceiling_mask = (gt_labels_loaded != 0)
                n_ceiling = int(np.sum(~ceiling_mask))
                n_total = len(ceiling_mask)
                print(f"[俯视模式] 基于语义标签过滤天花板(标签0)，移除 {n_ceiling}/{n_total} 个天花板点")
            else:
                # 没有语义标签时，基于Z轴高度过滤（天花板通常是最高点）
                z_values = coords[:, 2]
                z_threshold = np.percentile(z_values, 90)
                ceiling_mask = (z_values <= z_threshold)
                n_ceiling = int(np.sum(~ceiling_mask))
                n_total = len(ceiling_mask)
                print(f"[俯视模式] 基于Z轴高度过滤天花板(阈值={z_threshold:.3f})，移除 {n_ceiling}/{n_total} 个点")

            # 应用掩码过滤坐标
            if np.sum(ceiling_mask) == 0:
                print("[俯视模式] 警告: 过滤后无剩余点云，跳过天花板过滤")
            else:
                coords = coords[ceiling_mask]
                if gt_labels_loaded is not None:
                    gt_labels_loaded = gt_labels_loaded[ceiling_mask]
        ...
        for window_type in window_types:
            ...
            if window_type == 'color':
                ...
                if gt_labels_loaded is not None:
                    gt_colors = get_label_colors(gt_labels_loaded, label_type='semantic')
                    window_configs.append(('gt', gt_colors, "真值语义"))
                    print_color_legend(gt_labels_loaded, "semantic")
                else:
                ...
                if os.path.exists(instance_path):
                    ...
                    if ceiling_mask is not None:
                        instance_labels = instance_labels[ceiling_mask]
...
def main():
    parser = argparse.ArgumentParser(
        description="灵活的点云可视化工具 (支持任意窗口组合，包括纯色窗口)。\n"
                    "视角控制功能：可通过 --view_angle 指定初始视角，\n"
                    "按 s 保存当前视角，按 r 重置视角，按 v 手动打印当前视角信息。\n"
                    "俯视模式：--LookDown 去除天花板点云并以俯视角度查看点云内部结构。",
    ...
    parser.add_argument("--LookDown", dest='look_down', action="store_true",
                        help="俯视模式：去除天花板点云并将视角设为俯视，方便查看点云内部结构")
