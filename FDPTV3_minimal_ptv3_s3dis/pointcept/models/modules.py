"""Shared point-aware module containers used by PTv3 and PDNorm.

These wrappers let ordinary PyTorch modules, SpConv blocks and Point-aware
modules participate in one sequential execution chain.
"""
...
class PointSequential(PointModule):
    ...
    def forward(self, input):
        for k, module in self._modules.items():
            # Point-aware modules consume and return the custom Point object.
            if isinstance(module, PointModule):
                input = module(input)
            # Sparse convolution modules update both sparse features and the
            # dense feature view stored on the Point wrapper.
                ...
                if isinstance(input, Point):
                    ...
                    input = module(input)
            # Plain PyTorch modules are applied to the dense feature tensor.
            else:
                if isinstance(input, Point):
