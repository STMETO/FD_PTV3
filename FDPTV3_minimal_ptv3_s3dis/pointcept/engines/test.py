"""Testing and final evaluation entrypoints.

The federated server calls into this module after the last aggregation round to
evaluate the final global model on the configured S3DIS test pipeline.
"""
...
class SemSegTester(TesterBase):
    def __init__(self, cfg, model=None, test_loader=None, verbose=False) -> None:
        ...
        if comm.is_main_process():
            try:
                ...
                tensorboard_dir = os.path.join(cfg.save_path, "tensorboard_test")
    ...
    def create_html_report(self, scene_metrics, save_path, cfg, is_final=False):
        """创建详细的HTML报告，包含交互式图表"""
        html_content = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
            ...
            <title>Scene-wise Evaluation Results</title>
        ...
        <body>
            <div class="container">
                <h1>Scene-wise Semantic Segmentation Evaluation Results</h1>
                
                <div class="status">
                    <strong>Status:</strong> 
                    """ + ("Complete" if is_final else "In Progress") + """ | 
                    <strong>Scenes Tested:</strong> <span id="sceneCount">""" + str(len(scene_metrics['scene_names'])) + """</span> | 
                    <strong>Last Updated:</strong> <span id="updateTime">""" + time.strftime("%Y-%m-%d %H:%M:%S") + """</span>
                </div>
                ...
                <div style="text-align: center; margin-top: 20px; color: #666;">
                    <em>Report will auto-update as testing progresses. Refresh page to see latest results.</em>
                </div>
            </div>
            
            <script>
                // 从Python传递的数据
                const sceneData = """ + json.dumps([
                    {"name": name, "accuracy": float(acc), "iou": float(iou)}
                    for name, acc, iou in zip(scene_metrics['scene_names'], 
                                             scene_metrics['accuracies'], 
                                             scene_metrics['ious'])
                ]) + """;
                ...
                // 创建准确率图表
                const accuracyTrace = {
                    x: sceneData.map(item => item.name),
                    y: sceneData.map(item => item.accuracy),
                    type: 'bar',
                    name: 'Accuracy',
                    marker: {
                ...
                const accuracyLayout = {
                    ...
                    yaxis: { 
                        title: 'Accuracy',
                        range: [0, Math.max(0.1, ...sceneData.map(item => item.accuracy)) * 1.1]
                    }
                };
                
                // 如果图表已存在，更新它；否则创建新图表
                if (document.getElementById('accuracyChart').data) {
                    Plotly.update('accuracyChart', [accuracyTrace], accuracyLayout);
                } else {
                ...
                const iouLayout = {
                    ...
                    yaxis: { 
                        title: 'mIoU',
                        range: [0, Math.max(0.1, ...sceneData.map(item => item.iou)) * 1.1]
                    }
                };
                
                // 如果图表已存在，更新它；否则创建新图表
                if (document.getElementById('iouChart').data) {
                    Plotly.update('iouChart', [iouTrace], iouLayout);
                } else {
                    Plotly.newPlot('iouChart', [iouTrace], iouLayout);
                }
        ...
        # 保存HTML文件
        if is_final:
            html_path = os.path.join(save_path, "scene_metrics_report_final.html")
        else:
            html_path = os.path.join(save_path, "scene_metrics_report_latest.html")
    ...
    def update_csv_file(self, scene_metrics, save_path):
        """更新CSV文件"""
        csv_path = os.path.join(save_path, "scene_metrics_latest.csv")
        with open(csv_path, 'w') as f:
            f.write("Scene Name,Accuracy,mIoU\n")
            for name, acc, iou in zip(scene_metrics['scene_names'], 
                                     scene_metrics['accuracies'], 
                                     scene_metrics['ious']):
                f.write(f"{name},{acc:.6f},{iou:.6f}\n")
        return csv_path
    ...
    def test(self):
        ...
        # 获取总场景数，用于进度显示
        total_scenes = len(self.test_loader)
        logger.info(f"Total scenes to test: {total_scenes}")
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            # 记录当前场景指标
            scene_metrics['accuracies'].append(acc)
            scene_metrics['ious'].append(iou)
            scene_metrics['scene_names'].append(data_name)

            # 记录到TensorBoard - 实时更新
            if self.writer is not None:
                self.writer.add_scalar('Per_Scene/Accuracy', acc, idx)
                self.writer.add_scalar('Per_Scene/mIoU', iou, idx)
                
                # 记录每个类别的IoU
                for class_idx in range(len(iou_class)):
                    if mask[class_idx]:  # 只记录有效的类别
                        class_name = self.cfg.data.names[class_idx]
                        self.writer.add_scalar(f'Per_Scene/Class_{class_name}_IoU', iou_class[class_idx], idx)
                
                # 记录渐进式指标曲线
                self.writer.add_scalar('Progressive/Accuracy', acc, idx)
                self.writer.add_scalar('Progressive/mIoU', iou, idx)
                
                # 记录累积平均值
                avg_acc = np.mean(scene_metrics['accuracies'])
                avg_iou = np.mean(scene_metrics['ious'])
                self.writer.add_scalar('Cumulative/Average_Accuracy', avg_acc, idx)
                self.writer.add_scalar('Cumulative/Average_mIoU', avg_iou, idx)
                
                # 强制刷新TensorBoard写入
                self.writer.flush()

            # 实时更新HTML报告和CSV文件（只在主进程）
            if comm.is_main_process():
                # 更新CSV文件
                csv_path = self.update_csv_file(scene_metrics, save_path)
                
                # 更新HTML报告（每次测试完一个场景都更新）
                html_path = self.create_html_report(scene_metrics, save_path, self.cfg, is_final=False)
                logger.info(f"Updated interim report: {html_path} (tested {idx+1}/{total_scenes} scenes)")
            ...
            # 如果只想测试部分场景，可以在这里添加提前退出的条件
            # 例如：if idx >= 3: break  # 只测试前4个场景
        ...
        if comm.is_main_process():
            ...
            # 记录最终指标到TensorBoard
            if self.writer is not None:
                self.writer.add_scalar('Final/mIoU', mIoU)
                self.writer.add_scalar('Final/mAcc', mAcc)
                self.writer.add_scalar('Final/allAcc', allAcc)
                
                # 记录每个类别的最终指标
                for class_idx in range(self.cfg.data.num_classes):
                    class_name = self.cfg.data.names[class_idx]
                    self.writer.add_scalar(f'Final/Class_{class_name}_IoU', iou_class[class_idx])
                    self.writer.add_scalar(f'Final/Class_{class_name}_Accuracy', accuracy_class[class_idx])
                
                self.writer.close()
                logger.info(f"TensorBoard logs saved to: {self.writer.log_dir}")

            # 生成最终报告
            html_path = self.create_html_report(scene_metrics, save_path, self.cfg, is_final=True)
            logger.info(f"Final report saved to: {html_path}")
            
            # 保存最终CSV
            csv_path = os.path.join(save_path, "scene_metrics_final.csv")
            with open(csv_path, 'w') as f:
                f.write("Scene Name,Accuracy,mIoU\n")
                for name, acc, iou in zip(scene_metrics['scene_names'], 
                                         scene_metrics['accuracies'], 
                                         scene_metrics['ious']):
                    f.write(f"{name},{acc:.6f},{iou:.6f}\n")
            logger.info(f"Final scene metrics CSV saved to: {csv_path}")
