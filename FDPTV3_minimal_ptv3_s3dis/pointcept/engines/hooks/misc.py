...
class InformationWriter(HookBase):
    """
    一个用于在训练期间记录和打印信息的钩子。
    它处理控制台日志、TensorBoard 和 Weights & Biases (wandb) 的日志记录。
    特别为联邦学习场景优化，确保本地客户端的训练曲线能够在 wandb 上连续绘制。
    """
    def __init__(self):
        # curr_iter 用于跟踪当前客户端在其本地训练中的迭代次数
        self.curr_iter = 0
        self.model_output_keys = []
    ...
    def before_train(self):
        ...
        # 为 wandb 定义自定义X轴，这有助于在UI中更好地组织图表
        if self.trainer.writer is not None and self.trainer.cfg.get("enable_wandb", False):
            # 为按 iteration 记录的指标定义X轴
            wandb.define_metric("local_train/step")
            wandb.define_metric("train_batch/*", step_metric="local_train/step")
            wandb.define_metric("lr", step_metric="local_train/step")
            
            # 为按 epoch 记录的指标定义X轴
            wandb.define_metric("local_train/epoch")
            wandb.define_metric("train_epoch/*", step_metric="local_train/epoch")

    def before_step(self):
        """在每个训练步骤（iteration）开始前被调用。"""
        self.curr_iter += 1
        # 准备控制台输出信息
        info = "Train: [{epoch}/{max_epoch}][{iter}/{max_iter}] ".format(
            epoch=self.trainer.epoch + 1,
    ...
    def after_step(self):
        """在每个训练步骤（iteration）结束后被调用。"""
        # 1. 从 trainer 获取并存储模型输出
        if "model_output_dict" in self.trainer.comm_info:
            model_output_dict = self.trainer.comm_info["model_output_dict"]
        ...
        # 2. 完成并打印控制台日志
        log_str = self.trainer.comm_info["iter_info"]
        for key in self.model_output_keys:
            log_str += f"{key}: {self.trainer.storage.history(key).val:.4f} "
        lr = self.trainer.optimizer.state_dict()["param_groups"][0]["lr"]
        log_str += f"Lr: {lr:.5f}"
        self.trainer.logger.info(log_str)
        self.trainer.comm_info["iter_info"] = "" # 重置，为下一步做准备

        # 3. 记录到 TensorBoard 和 Wandb
        if self.trainer.writer is not None:
            # 计算全局唯一的、连续的迭代步骤
            current_round = self.trainer.cfg.get("current_round", 0)
            steps_per_round = self.trainer.max_epoch * len(self.trainer.train_loader)
            global_iter = current_round * steps_per_round + self.curr_iter

            # 记录到 TensorBoard
            self.trainer.writer.add_scalar("lr", lr, global_iter)
            for key in self.model_output_keys:
                self.trainer.writer.add_scalar(
                    "train_batch/" + key,
                    self.trainer.storage.history(key).val,
    ...
    def after_epoch(self):
        """在每个本地训练的 epoch 结束后被调用。"""
        # 1. 打印周期平均指标到控制台
        epoch_info = "Train epoch summary: "
        for key in self.model_output_keys:
            epoch_info += f"{key}: {self.trainer.storage.history(key).avg:.4f} "
        self.trainer.logger.info(epoch_info)

        # 2. 记录周期平均指标到 TensorBoard 和 Wandb
        if self.trainer.writer is not None:
            # 计算全局唯一的、连续的周期步骤
            current_round = self.trainer.cfg.get("current_round", 0)
            max_epoch_per_round = self.trainer.max_epoch
            global_epoch = current_round * max_epoch_per_round + self.trainer.epoch + 1
            ...
            if self.trainer.cfg.get("enable_wandb", False):
                log_dict = {}
                for key in self.model_output_keys:
                    log_dict[f"train_epoch/{key}"] = self.trainer.storage.history(key).avg
