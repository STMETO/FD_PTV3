"""S3DIS dataset partitioning strategy for federated learning.

The minimal project uses Area-based partitions so each client receives one or
more whole S3DIS areas instead of randomly mixed rooms.
"""
...
class S3DISSplitter(BaseDatasetSplitter):
    ...
    def get_user_split(self, user_id, num_users, **kwargs):
        # Areas are assigned contiguously and any remainder is distributed to
        # the first few users. This keeps the logic deterministic and easy to
        # audit in logs.
