"""Default federated orchestration pipeline for the minimal project.

This module is the control tower of the extracted codebase. It wires together:

1. runtime/config normalization from Pointcept,
2. global model construction,
3. per-round client training,
4. server-side aggregation and validation,
5. final testing after the last federated round.
"""
...
class DefaultFederatedServer(BaseFederatedServer):
    ...
    def _init_global_model(cfg):
        # Build a temporary trainer only to materialize the model with exactly
        # the same code path as local clients. We then discard the trainer and
        # keep only the initialized global network.
    ...
    def _run_round(
        ...
        # Serialize the current global state once at the start of each round so
        # every client receives an identical snapshot.
        ...
        if strategy.server_lr_scheduler and hasattr(strategy, "update_lr"):
            # Server-side scheduler updates are round-based rather than
            # minibatch-based, so we step them once right before aggregation.
    ...
    def _finalize(net_glob, cfg, save_path, resume_file, glogger):
        ...
        # Final test logs are written into an isolated directory so training and
        # post-training evaluation outputs never overwrite each other.
