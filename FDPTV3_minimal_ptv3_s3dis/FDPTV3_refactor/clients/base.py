"""Base Flower client used by the minimal federated training project.

Each client is responsible for three things only:

1. derive a user-specific config and data split,
2. run one local training phase with the shared trainer implementation,
3. return the updated model weights to the server.
"""
...
class BaseFedClient(fl.client.NumPyClient):
	...
	def _prepare_user_config(self, round_idx):
		...
		# Data splitting is resolved here so the same global config can be reused
		# for all clients without mutating shared state.
	...
	def _init_model(self, user_cfg, parameters, round_idx):
		...
		# Local training intentionally reuses the same trainer implementation as a
		# standalone Pointcept run. This keeps optimizer/scheduler/hook behavior
		# consistent between centralized and federated execution.
