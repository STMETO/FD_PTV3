...
class FedServerCosineAnnealingMomentum(FedServerMomentumScheduler):
    ...
    def state_dict(self):
        d = super().state_dict()
        d.update({'total_rounds': self.total_rounds, 'min_beta': self.min_beta, 'max_beta': self.max_beta})
        return d
...
class FedServerLinearWarmupMomentum(FedServerMomentumScheduler):
    ...
    def state_dict(self):
        d = super().state_dict()
        d.update({'warmup_rounds': self.warmup_rounds, 'warmup_start_beta': self.warmup_start_beta,
                   'max_beta': self.max_beta, 'final_beta': self.final_beta})
        return d
...
class FedServerLinearDecayMomentum(FedServerMomentumScheduler):
    ...
    def state_dict(self):
        d = super().state_dict()
        d.update({'decay_rounds': self.decay_rounds, 'decay_start_beta': self.decay_start_beta,
                   'min_beta': self.min_beta, 'final_beta': self.final_beta})
        return d
...
def build_fed_server_momentum_scheduler(scheduler_config, total_rounds=None):
    ...
    if not isinstance(scheduler_config, dict):
        raise TypeError(f"scheduler_config must be a dict")
