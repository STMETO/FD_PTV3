"""
FD_PTV3 FedAvg S3DIS 配置示例
=============================
配置格式兼容原 FDPTV3 的 config.py，可以直接使用。
与 exp/s3dis/251004_FedAvg_FA_ampF_r100_u3_e9_bs03_gs002/config.py 逻辑一致。

用法:
    python -m FD_PTV3.fd_train --config-file FD_PTV3/configs/fedavg_s3dis.py
"""
...
test_only = False
seed = 27567705
save_path = 'exp/s3dis/fd_fedavg_test'
num_worker = 1
batch_size = 3
...
# ================================================================
# 联邦学习配置 (兼容原 FDPTV3 格式)
# ================================================================
federated = dict(
    num_users=3,
    total_rounds=100,
    users_per_gpu=1,
    msg='FD_PTV3 Flower FedAvg on S3DIS',
    aggregation_method='FedAvg',

    # 客户端配置
    client=dict(
        type='MarkovFedClient',     # BaseFedClient | MarkovFedClient
        aggre_mode='FedMarkovAvg',  # FedAvg | FedMarkovAvg | FedBinAvg
    ...
    hyperparameters=dict(
        ...
        fedavgm=dict(
            ...
            #     type='FedServerCosineAnnealingMomentum',
            #     initial_beta=0.9,
            #     min_beta=0.1,
            # ),
        ),
