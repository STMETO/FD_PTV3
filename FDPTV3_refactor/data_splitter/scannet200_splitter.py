...
class ScanNet200Splitter(BaseDatasetSplitter):
    ...
    def get_user_split(self, user_id, num_users, **kwargs):
        ...
        try:
            all_scenes = [
                item for item in os.listdir(train_data_path)
                if os.path.isdir(os.path.join(train_data_path, item)) and item.startswith("scene")
            ]
            all_scenes.sort()
            ...
            for scene in all_scenes:
                ...
                scene_groups.setdefault(scene_prefix, []).append(scene)
            ...
            with open(lr_file_path, "w", encoding="utf-8") as file:
                for scene in user_scenes:
                    file.write(f"{scene}\n")

            if self.glogger:
                self.glogger.info(
                    f"用户{to_display_user(user_id)}分配到 {len(user_scene_prefixes)} 个场景前缀，{len(user_scenes)} 个具体场景"
                )

            return lr_file_path
        except Exception as exc:
            if self.glogger:
                self.glogger.error(f"获取ScanNet200场景列表失败: {exc}")
            return ""
