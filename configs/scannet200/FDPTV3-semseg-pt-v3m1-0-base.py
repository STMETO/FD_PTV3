...
wandb_project = "FDPTV3_scannet" # my
...
federated = dict(
    # --- 核心参数 ---
    num_users=20,                # 客户端数量
    total_rounds=100,           # 全局训练的总轮数
    users_per_gpu= 1,           # 每块 GPU 上并行运行的客户端数量，可根据显存调整
    msg="FedAvgM测试",
    
    # --- 算法选择 ---
    # 可选项: 'FedAvg', 'FedAvgM', 'FedProx', 'FedAdam'
    aggregation_method="FedAvgM", 

    # --- 数据划分策略 ---
    # data_split_strategy=dict(
    #     # ScanNet200 数据划分配置
    #     scannet200=dict(
    #         method="by_scene_prefix",          # 按场景划分
    #         train_dir="train",          # 训练数据目录
    #         validation_split="val",     # 验证集划分
    #         random_seed=42,             # 随机种子，确保划分可重复
    #     )
    # ),

    data_split_strategy=dict(
        ScanNet200Dataset=dict(  # 驼峰命名
            type="ScanNet200Splitter",
            train_dir="train",
            random_seed=42,
        ),
    ),
...
data_root = "data/ScanNet/process_data"
